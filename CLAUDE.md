...
- Graph is per-service, two hops (`getServiceNeighborhood`); callers read from `toRelationships.calls`. "Test connection" probes every scope; the `entities.write` probe POSTs tags to a nonexistent entity ID (matches nothing).
